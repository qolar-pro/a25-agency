...
export default function App() {
    ...
    if (stored && stored in LANGUAGE_DETAILS) {
      ...
      try {
        ...
        if (tz.includes("Skopje")) {
          ...
          detected = "IT";
        } else if (tz.includes("Istanbul") || tz.includes("Ankara")) {
          detected = "TR";
        } else if (tz.includes("Belgrade") || tz.includes("Sarajevo") || tz.includes("Zagreb") || tz.includes("Podgorica")) {
          detected = "SR";
  ...
  const industries = [
    { key: 'construction', label: t.optConstruction, desc: 'Skilled trade & site labor sourcing.', icon: HardHat, iconBg: 'bg-blue-50', iconColor: 'text-blue-600' },
    { key: 'hospitality', label: t.optHospitality, desc: 'Hospitality & guest service staffing.', icon: UtensilsCrossed, iconBg: 'bg-amber-50', iconColor: 'text-amber-600' },
    { key: 'agriculture', label: t.optAgriculture, desc: 'Seasonal & permanent field labor.', icon: Sprout, iconBg: 'bg-emerald-50', iconColor: 'text-emerald-600' },
    { key: 'manufacturing', label: t.optManufacturing, desc: 'Warehouse & production line staffing.', icon: Warehouse, iconBg: 'bg-zinc-100', iconColor: 'text-zinc-700' },
    { key: 'generic', label: t.optGeneric || 'Generic Workers', desc: 'General labor for any role or site.', icon: Users, iconBg: 'bg-indigo-50', iconColor: 'text-indigo-600' },
  ];
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-zinc-200/80 shadow-xs">
        <div className="max-w-6xl 2xl:max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 lg:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* Elegant Language selector supporting all 25 languages */}
            <div className="flex items-center bg-zinc-100 border border-zinc-200 p-0.5 rounded-lg text-xs font-mono">
              ...
              >
                {!['EN', 'MK', 'AL', 'DE'].includes(language) && (
                  <option value={language}>
                    {LANGUAGE_DETAILS[language].flag} {LANGUAGE_DETAILS[language].native} ({language})
                  </option>
                )}
                <optgroup label="Primary Channels">
                  <option value="EN">🇬🇧 English</option>
                  <option value="MK">🇲🇰 Македонски</option>
                  <option value="AL">🇦🇱 Shqip</option>
                  <option value="DE">🇩🇪 Deutsch</option>
                </optgroup>
                <optgroup label="All Channels (Top 25)">
                  {Object.entries(LANGUAGE_DETAILS)
                    .filter(([code]) => !['EN', 'MK', 'AL', 'DE'].includes(code))
                    .map(([code, details]) => (
